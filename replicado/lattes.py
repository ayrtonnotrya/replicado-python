...
class Lattes:
    ...
    @staticmethod
    def id(codpes: int) -> Union[str, bool]:
        """
    ...
    @staticmethod
    def obter_zip(codpes: int) -> Union[bytes, bool]:
        """
    ...
    @staticmethod
    def obter_xml(codpes: int) -> Union[str, bool]:
        """
    ...
    @staticmethod
    def obter_json(codpes: int) -> Union[str, bool]:
        """
    ...
    @staticmethod
    def obter_array(codpes: int) -> Union[Dict[str, Any], bool]:
        """
    ...
    @staticmethod
    def listar_premios(codpes: int, lattes_array: Optional[Dict[str, Any]] = None) -> Union[list, bool]:
        """
        ...
        if 'PREMIOS-TITULOS' in premios_data:
            ...
            for p in premios:
                ...
                if not attrs or 'NOME-DO-PREMIO-OU-TITULO' not in attrs:
                    return False
                nome_premios.append(f"{attrs['NOME-DO-PREMIO-OU-TITULO']} - Ano: {attrs['ANO-DA-PREMIACAO']}")
            return nome_premios
        return False

    @staticmethod
    def retornar_resumo_cv(codpes: int, idioma: str = 'pt', lattes_array: Optional[Dict[str, Any]] = None) -> Union[str, bool]:
        """
    ...
    @staticmethod
    def retornar_data_ultima_atualizacao(codpes: int) -> Union[str, bool]:
        """
    ...
    def listar_autores(array_autores: list) -> list:
        ...
        if array_autores:
            ...
            for autor in array_autores:
                ...
                attrs = autor.get('@attributes', autor) # Fallback to self if attributes mixed?
            ...
            aux_autores.sort(key=lambda x: int(x['ORDEM-DE-AUTORIA']) if x['ORDEM-DE-AUTORIA'] else 999)
            return aux_autores
        return False

    @staticmethod
    def verificar_filtro(tipo: str, ano: int, limit_ini: int, limit_fim: int, i: int) -> bool:
        """
    ...
    def listar_artigos(
        codpes: int, 
        lattes_array: Optional[Dict[str, Any]] = None, 
        tipo: str = 'registros', 
        limit_ini: int = 5, 
        limit_fim: Optional[int] = None
    ) -> Union[list, bool]:
        """
        ...
        artigos = get_path(lattes, 'PRODUCAO-BIBLIOGRAFICA.ARTIGOS-PUBLICADOS.ARTIGO-PUBLICADO')
        if not artigos:
            return False
        ...
        def sort_key(a):
            try:
                ...
                return int(get_path(a, 'DADOS-BASICOS-DO-ARTIGO.@attributes.ANO-DO-ARTIGO', 0))
            except Exception:
                return 0
        ...
        for art in artigos:
            ...
            def get_attr(node, key, default=""):
                return node.get('@attributes', {}).get(key, default) if isinstance(node, dict) else default

            aux_artigo = {
                'SEQUENCIA-PRODUCAO': get_attr(art, 'SEQUENCIA-PRODUCAO', 0),
                'TITULO-DO-ARTIGO': get_attr(dados_basicos, 'TITULO-DO-ARTIGO'),
                'TITULO-DO-PERIODICO-OU-REVISTA': get_attr(detalhamento, 'TITULO-DO-PERIODICO-OU-REVISTA'),
            ...
            if not Lattes.verificar_filtro(tipo, aux_artigo['ANO'], limit_ini, limit_fim, i):
                continue
    ...
    @staticmethod
    def listar_linhas_pesquisa(codpes: int, lattes_array: Optional[Dict[str, Any]] = None) -> Union[list, bool]:
        """
        ...
        linhas = []
        atuacoes = get_path(lattes, 'DADOS-GERAIS.ATUACOES-PROFISSIONAIS.ATUACAO-PROFISSIONAL')
        ...
        if atuacoes:
            ...
            for ap in atuacoes:
                pesquisas = get_path(ap, 'ATIVIDADES-DE-PESQUISA-E-DESENVOLVIMENTO.PESQUISA-E-DESENVOLVIMENTO')
                if pesquisas:
                    ...
                    for p in pesquisas:
                        ...
                        if lps:
                            ...
                            for lp in lps:
                                titulo = get_path(lp, '@attributes.TITULO-DA-LINHA-DE-PESQUISA')
                                if titulo:
    ...
    def listar_livros_publicados(
        ...
        for l in livros:
            i += 1
            dados_basicos = l.get('DADOS-BASICOS-DO-LIVRO', {})
            detalhamento = l.get('DETALHAMENTO-DO-LIVRO', {})
            autores = l.get('AUTORES', [])
            ...
            if not Lattes.verificar_filtro(tipo, aux_livro['ANO'], limit_ini, limit_fim, i):
                continue
    ...
    def listar_capitulos_livros(
        ...
        for cap in capitulos:
            ...
            aux_capitulo = {
                'TITULO-DO-CAPITULO-DO-LIVRO': get_attr(dados_basicos, 'TITULO-DO-CAPITULO-DO-LIVRO'),
            ...
            if not Lattes.verificar_filtro(tipo, aux_capitulo['ANO'], limit_ini, limit_fim, i):
                continue
    ...
    def listar_trabalhos_anais(
        ...
        for anais in aux_trabalhos:
            ...
            aux_anais = {
                ...
                'TITULO-DOS-ANAIS-OU-PROCEEDINGS': get_attr(detalhamento, 'TITULO-DOS-ANAIS-OU-PROCEEDINGS'),
            ...
            if not Lattes.verificar_filtro(tipo, aux_anais['ANO'], limit_ini, limit_fim, i):
                continue
    ...
    def listar_trabalhos_tecnicos(
        ...
        for t in trabalhos:
            ...
            aux_trabalho_tec = {
                ...
                'ANO': get_attr(dados_basicos, 'ANO'),
                'INSTITUICAO-FINANCIADORA': get_attr(detalhamento, 'INSTITUICAO-FINANCIADORA'),
                'AUTORES': aux_autores
            }
            
            if not Lattes.verificar_filtro(tipo, aux_trabalho_tec['ANO'], limit_ini, limit_fim, i):
                continue
    ...
    def listar_apresentacao_trabalho(
        ...
        for ap in apresentacoes:
            ...
            if not Lattes.verificar_filtro(tipo, aux_ap['ANO'], limit_ini, limit_fim, i):
                continue
    ...
    def listar_organizacao_evento(
        ...
        for ev in eventos_raw:
            ...
            aux_evento = {
                ...
                'INSTITUICAO-PROMOTORA': get_attr(detalhamento, 'INSTITUICAO-PROMOTORA'),
                'SEQUENCIA-PRODUCAO': get_attr(ev, 'SEQUENCIA-PRODUCAO'),
                'AUTORES': aux_autores
            }
            
            if not Lattes.verificar_filtro(tipo, aux_evento['ANO'], limit_ini, limit_fim, i):
                continue
    ...
    def listar_outras_producoes_tecnicas(
        ...
        for outro in outras_raw:
            ...
            if not Lattes.verificar_filtro(tipo, aux_outro['ANO'], limit_ini, limit_fim, i):
                continue
    ...
    @staticmethod
    def obter_livre_docencia(codpes: int, lattes_array: Optional[Dict[str, Any]] = None) -> Union[list, bool]:
        """
    ...
    def listar_cursos_curta_duracao(
        ...
        for c in cursos_raw:
            i += 1
            dados_basicos = c.get('DADOS-BASICOS-DE-CURSOS-CURTA-DURACAO-MINISTRADO', {})
            ...
            aux_curso = {
                ...
                'INSTITUICAO-PROMOTORA-DO-CURSO': get_attr(detalhamento, 'INSTITUICAO-PROMOTORA-DO-CURSO'),
                'AUTORES': aux_autores
            }
            
            if not Lattes.verificar_filtro(tipo, aux_curso['ANO'], limit_ini, limit_fim, i):
                continue
    ...
    def listar_relatorio_pesquisa(
        ...
        for rel in relatorios_raw:
            ...
            if not Lattes.verificar_filtro(tipo, aux_relatorio['ANO'], limit_ini, limit_fim, i):
                continue
    ...
    def listar_material_didatico_instrucional(
        ...
        for mat in materiais_raw:
            i += 1
            dados_basicos = mat.get('DADOS-BASICOS-DO-MATERIAL-DIDATICO-OU-INSTRUCIONAL', {})
            ...
            if not Lattes.verificar_filtro(tipo, aux_material['ANO'], limit_ini, limit_fim, i):
                continue
    ...
    def listar_outras_producoes_bibliograficas(
        ...
        demais = get_path(lattes, 'PRODUCAO-BIBLIOGRAFICA.DEMAIS-TIPOS-DE-PRODUCAO-BIBLIOGRAFICA')
        if not demais:
            return False
        ...
        if outras_prod_raw:
            ...
            for o in outras_prod_raw:
                ...
                if not Lattes.verificar_filtro(tipo, aux_o['ANO'], limit_ini, limit_fim, i):
                    continue
                outras.append(aux_o)
        ...
        tipo_outras = [
            ...
            {'path': 'DO-PREFACIO-POSFACIO', 'key': 'PREFACIO-POSFACIO', 'label': 'Prefácio, Pósfacio'},
            {'path': 'DA-PARTITURA', 'key': 'PARTITURA-MUSICAL', 'label': 'Partitura Musical'},
        ]
        ...
        for t_spec in tipo_outras:
            ...
            if prods:
                ...
                for p in prods:
                    ...
                    label_tipo = f"{t_spec['label']}/{node_tipo.capitalize()}" if node_tipo else t_spec['label']
                    ...
                    if not Lattes.verificar_filtro(tipo, aux_p['ANO'], limit_ini, limit_fim, i):
                        continue
                    outras.append(aux_p)
    ...
    @staticmethod
    def retornar_banca_mestrado(codpes: int, lattes_array: Optional[Dict[str, Any]] = None) -> Union[list, bool]:
        """
        ...
        nome_bancas = []
        for b in bancas:
            titulo = get_path(b, 'DADOS-BASICOS-DA-PARTICIPACAO-EM-BANCA-DE-MESTRADO.@attributes.TITULO')
            if titulo:
                nome_bancas.append(titulo)
        return nome_bancas if nome_bancas else False

    @staticmethod
    def retornar_banca_doutorado(codpes: int, lattes_array: Optional[Dict[str, Any]] = None) -> Union[list, bool]:
        """
        ...
        nome_bancas = []
        for b in bancas:
            titulo = get_path(b, 'DADOS-BASICOS-DA-PARTICIPACAO-EM-BANCA-DE-DOUTORADO.@attributes.TITULO')
            candidato = get_path(b, 'DETALHAMENTO-DA-PARTICIPACAO-EM-BANCA-DE-DOUTORADO.@attributes.NOME-DO-CANDIDATO')
