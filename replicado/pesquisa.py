...
class Pesquisa:
    ...
    def listar_iniciacao_cientifica(
        ...
        for ic in results:
            ...
            bolsa_res = DB.fetch(
                q_bolsa, {"codpes": ic["aluno"], "codprj": ic["cod_projeto"]}
            )
    ...
    def contar_pd_por_ano(statuses: list[str] | None = None) -> dict[int, int]:
        ...
        if not statuses:
            ...
            statuses = [row["staatlprj"] for row in stats_res if row["staatlprj"]] or [
                "Ativo",
                "Aprovado",
            ]
        ...
        query = query.replace("__codundclg__", unidades).replace(
            "__statuses__", status_str
        )
