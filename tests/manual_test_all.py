...
def run_tests() -> None:
    ...
    codundclg = (
        int(codundclg_env.split(",")[0]) if "," in codundclg_env else int(codundclg_env)
    )
    ...
    if unidades:
        ...
        if locais:
            ...
            print(
                f"Procurar local part {part_cod}: {len(Estrutura.procurar_local(part_cod, codund))}"
            )
    ...
    if alunos_grad:
        ...
        print(
            "Obter curso ativo:",
            Graduacao.obter_curso_ativo(codpes).get("nomcur", "N/A"),
        )
        ...
        if curso:
            ...
            if codcur:
                ...
                if disciplinas and len(disciplinas) > 0:  # Walrus if python 3.8+
                    ...
                    print(
                        f"Nome disciplina {coddis}:", Graduacao.nome_disciplina(coddis)
                    )
                    print(
                        f"Créditos disciplina {coddis}:",
                        Graduacao.creditos_disciplina(coddis),
                    )
                ...
                # Médias
                print(
                    f"Média Ponderada (Suja): {Graduacao.obter_media_ponderada_suja(codpes)}"
                )
                print(
                    f"Média Ponderada (Limpa): {Graduacao.obter_media_ponderada_limpa(codpes)}"
                )

                # Grade Horária
                print(
                    f"Grade Horária atual: {len(Graduacao.obter_grade_horaria(codpes))}"
                )

                # Intercâmbio
                try:
                    print(
                        f"Intercâmbios (geral): {len(Graduacao.listar_intercambios())}"
                    )
                    print(
                        f"Intercâmbio aluno: {len(Graduacao.obter_intercambio_por_codpes(codpes))}"
                    )
                ...
                # Verificações
                print(
                    f"Graduado na unidade? {Graduacao.verificar_pessoa_graduada_unidade(codpes)}"
                )
                print(
                    f"Coordenador? {Graduacao.verificar_coordenador_curso_grad(codpes)}"
                )
    ...
    if "codpes" in locals() and codpes:
        ...
        if vinculos:
            ...
            print(
                f"Total vínculo '{tipvinext}':",
                Pessoa.total_vinculo(tipvinext, codundclg),
            )
    ...
    if programas:
        ...
        if orientadores:
            try:
                ...
                print(
                    f"Orientandos do prof {codpes_ori}:",
                    len(Posgraduacao.listar_orientandos_ativos(codpes_ori)),
                )
            except ValueError:
                pass
        ...
        if oferecimentos:
            ...
            print(
                f"Disciplina {sgldis}:", Posgraduacao.disciplina(sgldis).get("nomdis")
            )
            print(
                "Oferecimento detalhe:",
                Posgraduacao.oferecimento(sgldis, numofe).get("dtainiofe"),
            )
            print(
                "Espaço turma:",
                len(Posgraduacao.espacoturma(sgldis, numseqdis, numofe)),
            )
            print(
                "Ministrantes:",
                len(Posgraduacao.ministrante(sgldis, numseqdis, numofe)),
            )
    ...
    if alunos_pos:
        ...
        print(
            "Obter Vínculo Pós:",
            Posgraduacao.obter_vinculo_ativo(codpes_pos).get("nomcur"),
        )
        ...
        if "codpes_ori" in locals():
            print(
                f"Orientandos Ativos (alias) do prof {codpes_ori}: {len(Posgraduacao.obter_orientandos_ativos(codpes_ori))}"
            )
            print(
                f"Orientandos Concluídos do prof {codpes_ori}: {len(Posgraduacao.listar_orientandos_concluidos(codpes_ori))}"
            )

        # Obter Defesas (do aluno?)
        # Search for a student with defenses (might be empty for active student)
        print(
            f"Defesas do aluno {codpes_pos}: {len(Posgraduacao.obter_defesas(codpes_pos))}"
        )
    ...
    if programas:
        ...
        print(
            f"Egressos da área {codare_test}: {len(Posgraduacao.egressos_area(codare_test))}"
        )
        print(
            f"Egressos por ano: {Posgraduacao.contar_egressos_area_agrupado_por_ano(codare_test)}"
        )
    ...
    if bens:
        ...
        numpat = bem["numpat"]
        print(
            f"Bem {numpat}: Is Informatica? {Bempatrimoniado.is_informatica(str(numpat))}"
        )
