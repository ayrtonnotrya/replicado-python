...
class TestBeneficio(unittest.TestCase):
    ...
    def test_listar_beneficios(self, mock_fetch):
    ...
    @patch('replicado.connection.DB.fetch_all')
    def test_listar_monitores_pro_aluno(self, mock_fetch):
    ...
    @patch('replicado.connection.DB.fetch_all')
    def test_listar_monitores_pro_aluno_string(self, mock_fetch):
        mock_fetch.return_value = [{'codpes': 123}]
        res = Beneficio.listar_monitores_pro_aluno("1, 2")
        self.assertEqual(len(res), 1)
        # Check call args
