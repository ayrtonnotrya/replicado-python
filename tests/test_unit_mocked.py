...
from replicado.utils import clean_string, remove_accents, dia_semana, data_mes, horario_formatado
...
# --- CONNECTION TESTS ---
def test_connection_missing_env():
...
def test_connection_success(mock_create_engine):
    DB._engine = None
    env_vars = {
        'REPLICADO_HOST': 'host',
        'REPLICADO_PORT': '1433',
...
def test_cria_filtro_busca():
    ...
    with patch('replicado.connection.DB.get_engine') as mock_engine_getter:
        ...
        # fetch
        mock_conn.execute.return_value.fetchone.return_value = MagicMock(_mapping={'a': ' b '})
        res = DB.fetch("SELECT 1")
        assert res['a'] == 'b'
        
        # fetch_all
        mock_conn.execute.return_value._mapping.items.return_value = {'a': ' b '}.items() # Mock row mapping items
...
def test_graduacao_methods(mock_fetch_all, mock_fetch):
    # Setup mocks
    mock_fetch.return_value = {'nompes': 'Teste', 'codpes': 123}
    mock_fetch_all.return_value = [{'codpes': 123, 'nompes': 'Teste', 'tipvin': 'ALUNOGR', 'sitatl': 'A', 'codundclg': '45'}]
    ...
    # listar_ativos
    mock_fetch_all.return_value = [{'codpes': 1, 'nompes': 'A'}] # Valid data for loop/return
    ...
    # media_ponderada
    mock_fetch_all.return_value = [{'creaul': 4, 'cretrb': 2, 'notfim2': '8.5', 'notfim': '8.5'}]
    ...
    mock_fetch_all.return_value = [{'coddis': 'D1', 'creaul': 4, 'cretrb': 0, 'notfim': '10'}]
    ...
    assert Graduacao.creditos_disciplinas_concluidas_aproveitamento_estudos_exterior(123, 45) == []
    ...
    mock_fetch_all.return_value = [
        ...
        {'creaul': 4, 'cretrb': 0, 'notfim2': 'abc', 'notfim': '10'}  # ValueError on float conversion
    ]
    Graduacao.obter_media_ponderada(123)
...
@patch('replicado.connection.DB.fetch')
@patch('replicado.connection.DB.fetch_all')
def test_posgraduacao_methods(mock_fetch_all, mock_fetch):
    # verifica
    mock_fetch_all.return_value = [{'codpes': 123, 'tipvin': 'ALUNOPOS', 'sitatl': 'A', 'codundclg': '45'}]
    ...
    mock_fetch.side_effect = [
        ...
        {'nomcur': 'X', 'nivpgm': 'ME', 'dtainivin': '2010', 'nomare': 'Y'} # obter_vinculo inner call
    ]
    ...
    with patch('replicado.posgraduacao.Posgraduacao.obter_vinculo_ativo') as m_vinc:
        m_vinc.return_value = {'nomcur': 'X', 'nivpgm': 'ME', 'dtainivin': '2010', 'nomare': 'Y'}
...
@patch('replicado.connection.DB.fetch')
@patch('replicado.connection.DB.fetch_all')
def test_pessoa_methods(mock_fetch_all, mock_fetch):
...
@patch('replicado.connection.DB.fetch')
@patch('replicado.connection.DB.fetch_all')
def test_estrutura_methods(mock_fetch_all, mock_fetch):
...
@patch('replicado.connection.DB.fetch')
@patch('replicado.connection.DB.fetch_all')
def test_bempatrimoniado_methods(mock_fetch_all, mock_fetch):
...
@patch('replicado.connection.DB.fetch')
@patch('replicado.connection.DB.fetch_all')
def test_coverage_gap_fillers(mock_fetch_all, mock_fetch):
    # Graduacao
    ...
    mock_fetch_all.return_value = [{'coddis': 'D1', 'creaul': 4, 'cretrb': 2, 'notfim': '10'}]
    Graduacao.listar_disciplinas_aluno(123)
