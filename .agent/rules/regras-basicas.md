...
2.  **Qualidade de Código (Ruff):** O projeto utiliza **Ruff** para linting e formatação. Antes de considerar uma tarefa concluída, garanta que `poetry run ruff check . --fix` e `poetry run ruff format .` foram executados e não há erros pendentes.
